...
var TestScenario = /** @class */ (function () {
  function TestScenario(featureFile, krakenApp) {
    ...
    this.executionId = this.featureFile.filePath
      .split("/")
      .pop()
      .replace(".feature", "");
    /// END MODIFICATION
        ...
        switch (_a.label) {
          case 0:
            if (!this.featureFile.hasRightSyntax()) {
              throw new Error(
                "ERROR: Verify feature file " +
                  this.featureFile.filePath +
                  " has one unique @user tag for each scenario"
              );
            }
            this.beforeExecute();
            this.execute();
  ...
  TestScenario.prototype.deleteSupportFilesAndDirectories = function () {
    FileHelper_1.FileHelper.instance().deleteFileInPathIfExists(
  ...
  TestScenario.prototype.deleteAllInboxes = function () {
    FileHelper_1.FileHelper.instance().deleteFilesWithGlobPattern(
      process.cwd() +
        "/" +
        Constants.KRAKEN_DIRECTORY +
        "/.*_" +
        Constants.INBOX_FILE_NAME
    );
  };
  ...
  TestScenario.prototype.sampleMobileDevices = function () {
    ...
    var numberOfRequiredMobileDevices =
  ...
  TestScenario.prototype.sampleWebDevices = function () {
    var numberOfRequiredWebDevices =
  ...
  TestScenario.prototype.allRegiresteredDevicesFinished = function () {
    ...
    var finished_ids = DeviceProcess_1.DeviceProcess.processesInState(
      Constants.PROCESS_STATES.finished
    );
    return (
      registered_ids.filter(function (registered_id) {
      ...
      return __generator(this, function (_a) {
        return [
          ...
          new Promise(function (resolve) {
            return _this.waitForAllProcessesToFinishOrTimeout(
              Date.now(),
              resolve
            );
          }),
  ...
  TestScenario.prototype.waitForAllProcessesToFinishOrTimeout = function (
    startTime,
    resolve
  ) {
